...
def traffic_roles(traffic_type: str) -> tuple[str, str]:
    if traffic_type == "voip":
        return "peer", "peer"

    if traffic_type in {"icmp", "web", "email", "video", "whatsapp"}:
        return "sender", "receiver"

    raise ValueError(f"Unsupported traffic type: {traffic_type}")


def run_capture(configuration: dict, duration: float, output: Path) -> None:
    ip_version = configuration["ip_version"]
    local_address, remote_address = get_addresses(ip_version)
    capture_filter = build_capture_filter(ip_version)
    role_a, role_b = traffic_roles(configuration["traffic_type"])
    
    controller = Controller(VM1, VM2)
    ...
    try:
        ...
        controller.configure(configuration, configuration)
        ...
        if "ESTABLISHED" not in status_a:
            raise RuntimeError("VM1 IKE_SA was not established.")
        if "ESTABLISHED" not in status_b:
            raise RuntimeError("VM2 IKE_SA was not established.")

        print("IPsec SA established on both agents.")

        write_status(stage="traffic_starting", status="running")
        print(f"Starting {configuration['traffic_type']} traffic...")
        ...
        if ipsec_started:
            ...
            try:
                ...
                print(f"Warning: failed to terminate IPsec: {exc}")

        if capture_started:
            controller.download_capture(output)

    print("\nValidating PCAP...")
    valid = validate_pcap(output, ip_version, local_address, remote_address)

    if not valid:
        write_status(stage="validation_failed", status="failed", output=str(output))
        raise RuntimeError("Generated PCAP failed validation.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a controlled IPsec experiment and generate a validated PCAP."
    )
    parser.add_argument("configuration", type=Path, help="Path to configuration.yaml")
    parser.add_argument("duration", type=float, help="Capture duration in seconds.")
    parser.add_argument("--output", type=Path, default=None, help="Output PCAP path.")
    ...
    if args.output is None:
        ...
        output = DEFAULT_OUTPUT_DIR / f"live_{timestamp}.pcap"
    else:
        output = args.output
    ...
    print(f"\nDuration: {args.duration:g}s")
    print(f"Output  : {output}\n")

    run_capture(configuration, args.duration, output)
